...
module DigimonsService { 
    ...
    export function getCrear(id:number,name:string,nombreTipo:string,fuerteTipo:string,debilTipo:string,img:string): DigimonI {
        const digimons: Array<DigimonI> = db;
        const obj: DigimonI={
            id:id,
            name:name,
    ...
    export function getFuerte(name:string): Array<DigimonI> {
        ...
        digimons.forEach(digimon => {
            const found = digimon.type.filter(e => e.weakAgainst === type1[0].strongAgainst);
            if (found.length > 0) {
    ...
    export function getDebil(name:string): Array<DigimonI> {
        const Digimones: Array<DigimonI> = db;
        let digimon = Digimones.filter(e=>e.name.toLowerCase()=== name.toLowerCase());
        if(digimon.length<1){
            throw "No se encontro el primer pokemon";
        }
        const type1 = digimon[0].type.filter(e=> e.name);
